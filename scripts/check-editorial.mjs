...
assert(revision.picks.length>0);assert.equal(new Set(revision.picks.map(x=>x.id)).size,revision.picks.length);
...
assert.equal((daily.match(/class="daily-story"/g)||[]).length,revision.picks.length);
...
console.log('PASS editorial: distinct editorial selections; 19 revisions; corrections and original publication dates.');

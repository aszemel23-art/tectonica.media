(()=>{
 ...
 dialog.innerHTML='<div class="viewer-toolbar"><span data-count aria-live="polite"></span><div><button type="button" data-full>На весь экран ⛶</button><button type="button" data-close aria-label="Закрыть просмотр">Закрыть ×</button></div></div><div class="viewer-stage"><img alt=""><button type="button" data-prev aria-label="Предыдущая фотография">‹</button><button type="button" data-next aria-label="Следующая фотография">›</button></div><div class="viewer-caption"></div>';
 ...
 if(hero){const img=hero.querySelector('img'),button=document.createElement('button');button.type='button';button.className='hero-enlarge';button.setAttribute('aria-label','Открыть фотографии на весь экран');img.before(button);button.append(img);const label=document.createElement('span');label.textContent='Смотреть фотографии · '+photos.length+' ⛶';button.append(label);button.onclick=()=>open(0,button);}
 if(figures.length){
